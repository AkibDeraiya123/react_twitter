...
class Profile extends React.Component {
  ...
  handleUpload(event) {
    ...
    event.preventDefault();
    const coki = cookie.load('userId');
    const userDetail = { fname: this.state.fname, lname: this.state.lname, phone: this.state.phone, user: coki };
  ...
  componentWillMount() {
    ...
    const self = this;
    const a = `http://localhost:5000/profile/${cookie.load('userId')}`;
  ...
  displayFollowerList(posts) {
    const list = posts.map((temp, index) => {
      ...
      const followurl = `/unfollow/${temp.id}`;
      return (
      ...
      <div>
        ...
        <div className="container profilecover">
          <div className="img-responsive col-md-12" >
            <img alt="This is profile pic"  src={this.state.profilePic}   />
          </div>
